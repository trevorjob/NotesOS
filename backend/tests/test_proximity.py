...
async def _register(
    ...
    *,
    ...
    """Register + OTP-verify a fresh user with proximity signals (phone-primary)."""
...
async def test_no_school_skips_proximity(client, register_user):
    """A creator with no school runs no check — even an identical course forks."""
    a = await _register(register_user)  # no school
    b = await _register(register_user)  # no school
...
async def test_first_course_at_school_always_creates(client, register_user):
    """Nothing to match against yet ⇒ the first course is created outright."""
    a = await _register(register_user, school_name=UNILAG)
    resp = await _create(client, a["headers"], code="CHM101", name="Chemistry 101")
    assert resp.status_code == 201, resp.text
    assert resp.json()["course"]["code"] == "CHM101"


async def test_same_school_offers_match(client, register_user):
    ...
    b = await _register(register_user, school_name=UNILAG)
...
async def test_force_forks_despite_match(client, register_user):
...
async def test_different_school_no_match(client, register_user):
    """School is a hard filter: an identical course at another school never matches."""
    a = await _register(register_user, school_name=UNILAG)
    await _create(client, a["headers"], code="CHM101", name="Chemistry 101")

    b = await _register(register_user, school_name=OAU)
    resp = await _create(client, b["headers"], code="CHM101", name="Chemistry 101")
    assert resp.status_code == 201, resp.text


async def test_program_overlap_ranks_first(client, register_user):
    ...
    physics_owner = await _register(register_user, school_name=UNILAG, program="Physics")
    phys_course = await _create(
        client, physics_owner["headers"], code="CHM101", name="Chemistry 101"
    )
    chem_owner = await _register(register_user, school_name=UNILAG, program="Chemistry")
    ...
    student = await _register(register_user, school_name=UNILAG, program="Chemistry")
...
async def test_offered_match_is_joinable(client, register_user):
