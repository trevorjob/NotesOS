...
def upgrade() -> None:
    ...
    op.create_index('ix_semesters_invite_code', 'semesters', ['invite_code'], unique=True)

    # --- SemesterRole enum ---
    semester_role_enum = postgresql.ENUM('OWNER', 'MEMBER', name='semesterrole', create_type=True)
    ...
    op.create_index('ix_semester_members_semester_id', 'semester_members', ['semester_id'], unique=False)
    op.create_index('ix_semester_members_user_id', 'semester_members', ['user_id'], unique=False)

    # --- Courses: add semester_id FK ---
    op.add_column('courses', sa.Column('semester_id', postgresql.UUID(as_uuid=True), nullable=True))
    ...
    answer_status_enum = postgresql.ENUM('CORRECT', 'PARTIAL', 'NEEDS_REVIEW', name='answerstatus', create_type=True)
    ...
    notification_type_enum = postgresql.ENUM(
        'TEST_GRADED', 'AI_SUMMARY_READY', 'INVITE_ACCEPTED', 'GENERAL',
