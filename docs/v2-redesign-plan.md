...
- [ ] **A0 · Auth: phone-primary migration.** `phone` → required + unique, OTP-verified (WhatsApp/
  SMS provider is the owner's call); `email` → nullable/optional; Google OAuth stays but the user
  still **enters + OTP-verifies a phone** (never inferred from Google). Do this **first** — it's
  the identity everything sits on, and the model change is cleaner before capture/session work
  piles on. *Green when:* signup/login work phone-first (+ OTP + OAuth path); email optional;
  migration autogenerated; tests. *(Foundational — powers reliable contact discovery.)*
